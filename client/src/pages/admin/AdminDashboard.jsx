...
export default function AdminDashboard() {
  ...
  const { stats, latestRegistrations, pendingRequestsList } = data;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border border-slate-200">
          <p className="text-sm font-medium text-slate-500">Pending Join Requests</p>
          <p className="mt-2 text-3xl font-bold text-indigo-600">{stats.pendingRequests || 0}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-8">
        <div className="bg-white rounded-lg shadow-sm border border-slate-200">
          <div className="px-6 py-4 border-b border-slate-200">
            <h2 className="text-lg font-semibold text-slate-800">Recent Pending Requests</h2>
          </div>
            ...
            {pendingRequestsList && pendingRequestsList.length > 0 ? (
              pendingRequestsList.map(req => (
                <div key={req._id} className="px-6 py-4 flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-slate-900">{req.requestedBy?.name || 'Unknown User'}</p>
                    <p className="text-xs text-slate-500">Project: {req.project?.title || 'Unknown Project'}</p>
                  </div>
              ...
              <div className="p-6 text-center text-slate-500">No pending join requests.</div>
            )}
          </div>
        </div>
